...
async def test_client(tmp_home):
    """Minimal FastAPI test client; yields ``(client, ctx)`` with context stubs."""
...
async def test_put_settings_with_provider_rebuilds_clients(test_client, tmp_home):
    ...
    original_chat_service = ctx.chat_service

    new_chat_fn = AsyncMock(return_value="new")
    new_embed = MagicMock()
    new_embed.aclose = AsyncMock()

    with patch("brn_daemon.services.make_chat_fn", return_value=(new_chat_fn, AsyncMock())) as mock_chat, \
         patch("brn_daemon.services.make_embed_client", return_value=new_embed):
    ...
    assert ctx.chat_service is not original_chat_service
...
async def test_put_settings_without_provider_skips_rebuild(test_client, tmp_home):
    ...
    client, ctx = test_client
    original = ctx.chat_service

    with patch("brn_daemon.services.make_chat_fn") as mock_chat:
        resp = await client.put("/settings", json={"capture_interval_seconds": 30})

    assert resp.status_code == 200
    mock_chat.assert_not_called()
    assert ctx.chat_service is original
